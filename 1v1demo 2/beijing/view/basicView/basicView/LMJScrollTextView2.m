...
- (void)setLmj_text:(id)lmj_text{
    if ([lmj_text isKindOfClass:[NSMutableAttributedString class]]) {
...
- (void)scrollWithNoSpaceByDirection:(NSNumber *)direction{
    ...
    // 处于当前页面
        ...
        NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:_textDataArr[_index]];
        //设置文字颜色
        [str addAttribute:NSForegroundColorAttributeName value:XZRGB(0xE14FA9) range:NSMakeRange(1, 7)];
        NSString *yuanStr = [_textDataArr[_index] componentsSeparatedByString:@"充值了"][1];
        yuanStr = [yuanStr substringToIndex:yuanStr.length - 1];
        [str addAttribute:NSForegroundColorAttributeName value:XZRGB(0xE14FA9) range:NSMakeRange([_textDataArr[_index] length] - yuanStr.length -1, yuanStr.length)];

        _currentScrollLabel.lmj_text = str;
        
        NSMutableAttributedString *nextstr = [[NSMutableAttributedString alloc] initWithString:_textDataArr[[self nextIndex:_index]]];
        //设置文字颜色
        [nextstr addAttribute:NSForegroundColorAttributeName value:XZRGB(0xE14FA9) range:NSMakeRange(1, 7)];
        NSString *nextyuan = [_textDataArr[[self nextIndex:_index]] componentsSeparatedByString:@"充值了"][1];
        nextyuan = [nextyuan substringToIndex:nextyuan.length - 1];
        [nextstr addAttribute:NSForegroundColorAttributeName value:XZRGB(0xE14FA9) range:NSMakeRange([_textDataArr[[self nextIndex:_index]] length] - nextyuan.length -1, nextyuan.length)];
        _standbyScrollLabel.lmj_text  = nextstr;
