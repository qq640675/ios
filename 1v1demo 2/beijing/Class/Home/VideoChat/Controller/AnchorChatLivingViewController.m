//
//  AnchorChatLivingViewController.m
//  beijing
//
...
@interface AnchorChatLivingViewController ()
...
//对方关闭了摄像头
//@property (nonatomic, strong) AnchorMatchingLivingTempView *livingTempView;
//主播数据
...
// 关闭用户视图
@property (nonatomic, strong) UIImageView *closeUserShade;
@property (nonatomic, strong) UIView *userView;//用于添加用用户视图的蒙版

@property (nonatomic, assign) BOOL isCamereMuted;//对方是否关闭了摄像头
...
@implementation AnchorChatLivingViewController
...
- (void)setupUI {
    ...
    if (_isMeCallOther) {
        ...
        _chatLiveMeCallOtherView.isUser = NO;
    } else {
        //被呼叫
        [self.view addSubview:self.chatLiveOtherCallMeView];
        _chatLiveOtherCallMeView.isUser = NO;
    }
...
//    [YLTapGesture addTaget:self sel:@selector(clickedLivingViewSwitchBtn) view:_chatLiveMeCallOtherView.switchBtn];
    ...
    [YLTapGesture addTaget:self sel:@selector(clickedFollowBtn) view:_liveView.anchorMatchingLivingPersonView.followBtn];
...
- (void)clickedFollowBtn {
    if (_liveView.personHandle.isFollow == 0) {
...
- (void)postDataWithFollow {
    ...
    WEAKSELF
    [YLNetworkInterface addAttention:[YLUserDefault userDefault].t_id coverFollowUserId:_userId block:^(BOOL isSuccess) {
        if (isSuccess) {
            weakSelf.liveView.personHandle.isFollow = 1;
            [weakSelf.liveView.anchorMatchingLivingPersonView.followBtn setTitle:@"已关注" forState:UIControlStateNormal];
        }
    }];
}
...
- (void)postDataWithFollowed {
    ...
    WEAKSELF
    [YLNetworkInterface cancelAtttention:[YLUserDefault userDefault].t_id coverFollowUserId:_userId  block:^(BOOL isSuccess) {
        if (isSuccess) {
            weakSelf.liveView.personHandle.isFollow = 0;
            [weakSelf.liveView.anchorMatchingLivingPersonView.followBtn setTitle:@"关注" forState:UIControlStateNormal];
        }
    }];
}
...
- (void)getDataWithAnthor {
    ...
    [YLNetworkInterface getUserInfoByIdUser:_userId block:^(personalCenterHandle *handle) {
        ...
        dataHandle.goldLevel = (int)handle.goldLevel;
        dataHandle.t_city = handle.t_city;
        dataHandle.balance = handle.balance;
        dataHandle.isFollow = handle.isFollow;
        
        weakSelf.personalHandle = dataHandle;
        
        if (weakSelf.isMeCallOther) {
            [weakSelf.chatLiveMeCallOtherView.chatLivePersonInfoView initWithData:weakSelf.personalHandle];
        } else {
            [weakSelf.chatLiveOtherCallMeView.chatLivePersonInfoView initWithData:weakSelf.personalHandle];
            weakSelf.chatLiveOtherCallMeView.tempLb.text = [NSString stringWithFormat:@"%@邀请您进行视频通话...",weakSelf.personalHandle.t_nickName];
            
            if (weakSelf.personalHandle.coverList.count > 0) {

                [weakSelf.chatLiveOtherCallMeView.coverImageView sd_setImageWithURL:[NSURL URLWithString:weakSelf.personalHandle.t_cover_img]];
            }
...
- (void)clickedLivingViewSwitchBtn {
    self.liveView.userMatchingLivingActionView.switchBtn.selected = !self.liveView.userMatchingLivingActionView.switchBtn.selected;
    [self checkIsNeedCloseUserView];
}

- (void)checkIsNeedCloseUserView {
    [self.closeUserShade removeFromSuperview];
    if (self.liveView.userMatchingLivingActionView.switchBtn.selected) {
        self.closeUserShade = [[UIImageView alloc] initWithFrame:self.userView.bounds];
        self.closeUserShade.image = [UIImage imageNamed:@"shadeView"];
        [self.userView addSubview:self.closeUserShade];
    }
}
...
- (void)clickedLivingViewEndBtn {
    if(self.sid){
        [YLNetworkInterface stopEvent:self.sid resourceId:self.resourceId success:^{
    ...
    [LXTAlertView dismiss:^{
        ...
        if (self.isChatLived) {
            [self dismissViewControllerAnimated:YES completion:^{
                [[NSNotificationCenter defaultCenter] postNotificationName:@"inputStarRateVCNoti" object:nil userInfo:@{@"id":[NSString stringWithFormat:@"%d", self.userId], @"roomId":[NSString stringWithFormat:@"%d", self.roomId]}];
            }];
        } else {
            [self dismissViewControllerAnimated:YES completion:^{
            }];
...
- (void)clickedLivingViewGiftBtn {
    ...
    [[MansionSendGiftView shareView] showWithUserId:_userId isPlayGif:YES];
...
- (void)clickedLivingViewSmallBlackView {
    ...
    if (!_isChangedViewTemp) {
        ...
        self.userView = _liveView.smallRightTopVideoBlackView;
        [self.cameraManager setLocalView:_locationVideoImageView];
        [[AgoraManager shareManager] setRemoteViewWithId:self.userId view:_liveView.smallRightTopVideoImageView];
    } else {
        //切换为用户大图，主播小图
        self.locationVideoImageView = _liveView.smallRightTopVideoImageView;
        self.selfView = _liveView.smallRightTopVideoBlackView;
        self.userView = _liveView.fullVideoBlackView;
        [self.cameraManager setLocalView:_locationVideoImageView];
        [[AgoraManager shareManager] setRemoteViewWithId:self.userId view:_liveView.fullVideoImageView];
    }
    ...
    if (self.isCamereMuted) {
        [_liveView setIsShowCloseCamera:YES isBigView:!_isChangedViewTemp isSelf:NO];
    } else {
        [_liveView setIsShowCloseCamera:NO isBigView:!_isChangedViewTemp isSelf:NO];
    }
}
...
- (void)startGCDTimer {
    ...
    dispatch_source_set_event_handler(_matchTimer, ^{
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //鉴黄截图
            if (self.t_screenshot_anchor_switch == 1 && self.t_screenshot_time_list.count > 0 && timeCount > 0) {
...
- (void)requestVideoState {
    [YLNetworkInterface getVideoStatusWithUserId:self.userId anchorId:[YLUserDefault userDefault].t_id roomId:self.roomId result:^(bool isSuccess) {
        if (!isSuccess) {
            //挂断
            [self clickedLivingViewEndBtn];
        }
...
- (void)addOcrLog:(NSString *)url keyword:(NSString *)keyword{
    NSDictionary *param = @{@"userId":[NSString stringWithFormat:@"%d", [YLUserDefault userDefault].t_id],
                            @"videoUserId":[NSString stringWithFormat:@"%d", self.userId],
                            @"videoAnchorUserId":[NSString stringWithFormat:@"%d", [YLUserDefault userDefault].t_id],
                            @"videoImgUrl": url,
                            @"keyword":keyword,
...
- (void)addOtherPulpShadeView {
    self.otherShade = [[PulpShadeView alloc] initWithSuperView:self.userView isSelf:NO isBig:!_isChangedViewTemp];
}

- (void)reSetMyPulpViewFrame {
    [self.selfShade changeSuperView:self.selfView isBig:_isChangedViewTemp];
    [self.otherShade changeSuperView:self.userView isBig:!_isChangedViewTemp];
}
        ...
        if ([elem isKindOfClass:[TIMCustomElem class]]) {
            ...
            if ([type isEqualToString:@"0"] || [type isEqualToString:@"1"]) {
                ...
                [giftImageView sd_setImageWithURL:[NSURL URLWithString:dic[@"gift_still_url"]] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
                    [self brushGift:self.liveView.anchorMatchingLivingPersonView.iconImageView.image giftImage:image name:self.liveView.anchorMatchingLivingPersonView.nickNameLb.text giftName:[NSString stringWithFormat:@"1个%@", dic[@"gift_name"]] userId:dic[@"gift_id"] count:[[NSString stringWithFormat:@"%@", dic[@"gift_number"]] intValue]];
                    [giftImageView removeFromSuperview];
                }];
...
//- (AnchorMatchingLivingTempView *)livingTempView {
//    if (!_livingTempView) {
//        _livingTempView = [[AnchorMatchingLivingTempView alloc] initWithFrame:CGRectMake(App_Frame_Width/2 - 80, APP_Frame_Height-175, 160, 30)];
//    }
//    return _livingTempView;
//}
...
- (void)callingSucessHandle {
    ...
    //创建单聊会话
    NSString *identifier = [NSString stringWithFormat:@"%d",self.userId+10000];
    self.c2cConversation = [[TIMManager sharedInstance] getConversation:TIM_C2C receiver:identifier];
    
    NSDictionary *dic = @{@"connectUserId":@(weakSelf.userId)};
    [YLUserDefault saveDic:dic];
}

#pragma mark - AgoraRtcEngineDelegate
- (void)rtcEngine:(AgoraRtcEngineKit * _Nonnull)engine didJoinedOfUid:(NSUInteger)uid elapsed:(NSInteger)elapsed {
    NSLog(@"_____agora user joined uid:%ld", uid);
    if (uid == self.userId){
        ...
        [self.cameraManager setLocalView:_locationVideoImageView];
        [self initWithTiLive];
        [YLNetworkInterface acquire:[NSString stringWithFormat:@"%d", self.roomId] success:^(NSString *rId) {
            self.resourceId = rId;
            [self startEvent];
        }];
    }
...
- (void)startEvent{
    NSString *subscribeAudioUids = [NSString stringWithFormat:@"%@,%@",[NSString stringWithFormat:@"%d",self.userId],[NSString stringWithFormat:@"%d",[YLUserDefault userDefault].t_id]];
    [YLNetworkInterface startEvent:self.resourceId cname:[NSString stringWithFormat:@"%d", self.roomId] token:self.token subscribeAudioUids:subscribeAudioUids success:^(NSString *data) {
        self.sid = data;
    }];
}

- (void)rtcEngine:(AgoraRtcEngineKit *)engine firstRemoteVideoDecodedOfUid:(NSUInteger)uid size: (CGSize)size elapsed:(NSInteger)elapsed {
    NSLog(@"_____agora user firstRemoteVideo uid:%ld", uid);
    [[AgoraManager shareManager] setRemoteViewWithId:uid view:_liveView.fullVideoImageView];
}

- (void)rtcEngine:(AgoraRtcEngineKit * _Nonnull)engine didOfflineOfUid:(NSUInteger)uid reason:(AgoraUserOfflineReason)reason {
    NSLog(@"_____agora user Offline uid:%ld  reason:%lu", uid, (unsigned long)reason);
    if (uid == self.userId) {
...
- (void)rtcEngine:(AgoraRtcEngineKit * _Nonnull)engine didVideoMuted:(BOOL)muted byUid:(NSUInteger)uid  {
    if (uid != self.userId) return;
    
    self.isCamereMuted = muted;
    if (muted) {
        [_liveView setIsShowCloseCamera:YES isBigView:!_isChangedViewTemp isSelf:NO];
    } else {
        [_liveView setIsShowCloseCamera:NO isBigView:NO isSelf:NO];
    }
//    if (muted) {
//        //关闭了视频流
//        //关闭右上角的视图,主播的视图变回满屏视图
//        [self.view addSubview:self.livingTempView];
//        [self.cameraManager setLocalView:_liveView.smallRightTopVideoImageView];
//        [[AgoraManager shareManager] setRemoteViewWithId:uid view:_liveView.fullVideoImageView];
//
//        _liveView.smallRightTopVideoBlackView.backgroundColor = [UIColor clearColor];
//        _liveView.fullVideoBlackView.backgroundColor = [UIColor blackColor];
//        _isChangedViewTemp = NO;
////        _liveView.smallRightTopVideoBlackView.userInteractionEnabled = NO;
//    } else {
//        //开启了视频流
//        [_livingTempView removeFromSuperview];
//        _liveView.smallRightTopVideoBlackView.backgroundColor = [UIColor clearColor];
