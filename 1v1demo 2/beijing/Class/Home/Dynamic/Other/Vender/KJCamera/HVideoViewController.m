...
- (void)setTipString:(NSString *)tipString {
    if (tipString.length > 0 && !_topTipLabel) {
        _topTipLabel = [UIManager initWithLabel:CGRectMake(15, SafeAreaTopHeight+30, App_Frame_Width-30, 80) text:self.tipString font:15 textColor:UIColor.whiteColor];
        _topTipLabel.numberOfLines = 0;
        NSShadow *shadowMsg = [NSShadow new];
        shadowMsg.shadowBlurRadius = 2.0;
        shadowMsg.shadowOffset = CGSizeMake(0, 0);
        shadowMsg.shadowColor = UIColor.blackColor;
        NSAttributedString *attrStringNsg = [[NSAttributedString alloc] initWithString:tipString attributes:@{NSShadowAttributeName:shadowMsg}];
        _topTipLabel.attributedText = attrStringNsg;
...
- (IBAction)onEnsureAction:(UIButton *)sender {
    ...
    if (self.saveVideoUrl) {
        ...
        [self getVideoInfoWithSourcePath:self.saveVideoUrl];
        if (self.takeBlock) {
            self.takeBlock(self.saveVideoUrl,self.coverImage,self.videoSeconds,self.videoSize);
        }
        ...
        [assetsLibrary writeVideoAtPathToSavedPhotosAlbum:self.saveVideoUrl completionBlock:^(NSURL *assetURL, NSError *error) {
            ...
            if (error) {
                ...
                [self getVideoInfoWithSourcePath:assetURL];
                if (weakSelf.takeBlock) {
                    weakSelf.takeBlock(assetURL,self.coverImage,self.videoSeconds,self.videoSize);
                }
        ...
        if (self.takeBlock) {
            self.takeBlock(self.takeImage,nil,0,0);
        }
...
-(void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error{
    ...
    if (self.isVideo) {
        ...
        NSLog(@"____________%@", outputFileURL);
        if (!self.player) {
    ...
    NSLog(@"_______shijian :%ld", self.seconds);
    if (self.isAuth == YES && self.seconds > 10) {
        [Utility hideProgressDialog];
        [LXTAlertView alertViewDefaultOnlySureWithTitle:@"温馨提示" message:@"视频不足5s，请重新拍摄" sureHandle:^{
            [self recoverLayout];
        }];
        return ;
    }
...
- (void)getVideoInfoWithSourcePath:(NSURL *)path {
    AVURLAsset *asset = [AVURLAsset assetWithURL:path];
    
    AVAssetImageGenerator *imageGenerator = [AVAssetImageGenerator assetImageGeneratorWithAsset:asset];
    imageGenerator.appliesPreferredTrackTransform = YES;    // 截图的时候调整到正确的方向
    NSError *error = nil;
    CMTime timeM = CMTimeMake(0,30);//缩略图创建时间 CMTime是表示电影时间信息的结构体，第一个参数表示是视频第几秒，第二个参数表示每秒帧数.(如果要获取某一秒的第几帧可以使用CMTimeMake方法)
    CMTime actucalTime; //缩略图实际生成的时间
    CGImageRef cgImage = [imageGenerator copyCGImageAtTime:timeM actualTime:&actucalTime error:&error];
    if (error) {
        NSLog(@"_______-error:%@", error);
        Plog(@"截取视频图片失败:%@",error.localizedDescription);
    }
    CMTimeShow(actucalTime);
    UIImage *image = [UIImage imageWithCGImage:cgImage];
